...
mice_model <- mice( data = imputation_data, m = 1, maxit = 3, seed = 500)
...
impute_covariate <- 
  c( "sex", "economic_level", "rural", "initiation_age", 
     "BMI_value",
     "alzheimer_disease", "angina", "back_pain", "burn_injuries", "cancer", "cardiac_insufficiency_heart_failure", 
...
Follow_OT_func <- function(input, specific){
  ...
  pre_data <- 
    ...
    mutate( current_combined_date = pmin(discontinuation_date, switch_date, departure_date))
...
#running function
OT_outcomes_list <- plyr::llply( specific_outcome, Follow_OT_func, input = First_billing_dataset) 
...
cox_dataset_interaction <-   list( overall = cox_interraction_ITT,
  ...
  mutate( group_label = case_when( outcome_label == "overall" ~ "overall", 
                                   ...
                                   outcome_label == "windex" ~ paste(as.character(windex), "dis", sep = ""),
...
#==================================================================#
# Current risks of study outcomes in the on-treatment analysis. 
#==================================================================#
Table2 <- 
  ITT_survial_summary %>% 
